...
This document and every GitHub Issue remain planning artifacts. Live repository
`main` and the applicable current versioned config and receipt remain the formal
authority. For navigation, begin with `CURRENT_STATUS.md` and its machine-readable
companion, then follow the project source-of-truth order in `AGENTS.md`.
...
Labels are optional triage metadata. This overlay neither creates GitHub labels
nor applies them automatically; issue and pull-request state never grants
authority.
