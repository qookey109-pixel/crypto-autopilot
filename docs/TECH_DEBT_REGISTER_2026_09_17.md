...
| TD-011 / P1 — Local reconciliation | PARTIALLY INTEGRATED | Review remaining C1–C3 candidates against live main; preserve recovery material; keep each salvage slice isolated | The original 78-path inventory remains in the dirty checkout. C4 signal-ingest hardening is in [PR #491](https://github.com/qookey109-pixel/crypto-autopilot/pull/491); C5 quality authority validation is in [PR #492](https://github.com/qookey109-pixel/crypto-autopilot/pull/492). Existing main dedupe, source-run binding, freshness and one-pointer `NO_CHANGE` behavior are preserved; no original files were removed or overwritten. |
...
| TD-013 / P1 — Delivery and PR review | CHECKPOINT EXTENDED | Before future delivery, recheck the exact live head/base, checks and merge state | The isolated signal-ingest and signal-quality slices are tracked in PRs #491 and #492. This register does not substitute for a live open-PR query or grant merge authority. |
