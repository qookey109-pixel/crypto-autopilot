...
CURRENT_OPERATIONS = Path("research/status/current-operations-v0-2.json")
...
def validate_current_operations(current: dict[str, Any]) -> None:
    if current.get("schema") != "qookey-current-operations-v0.2":
    ...
    if pionex.get("current_main_materialization_status") != "PENDING_MANUAL_DISPATCH":
        raise RuntimeError("Pionex current-main validation state changed")
...
def overlay_current_operations(
    ...
    _upsert_status_item(
        ...
        detail=f"Run {core100['training_run_id']} completed：100 symbols / 18,235,427 rows / 249,228 examples。",
        status="COMPLETED",
    )
    _upsert_status_item(
        pipeline,
        name="Core100 Threshold Replay",
        detail=f"Run {replay['run_id']} completed；0.50-0.55 無 supported threshold change，configured threshold 保持不變。",
    ...
    _upsert_status_item(
        ...
        detail=(
            "PR #321 boundary fix 已進 main；current-main materialization 仍等待手動 dispatch。"
    ...
    _upsert_status_item(
        ...
        detail="2026-09-04 02:00Z 至 2026-09-12 03:59:59.999Z 的 bounded metadata window 已結束；保留歷史 lineage，不是目前 active capture path。",
        status="HISTORICAL",
    )
    ...
    _upsert_status_item(
        gates,
        name="Pionex Current-Main Validation",
        detail="Current main 已含 PR #321；新的 validation materialization 尚待 manual dispatch，未完成前不得宣稱 current-main validation complete。",
        status="PENDING_MANUAL_DISPATCH",
        tone="pending",
        critical=True,
    )
    ...
    dashboard["snapshotLabel"] = (
        ...
        "V0.12 HISTORICAL / Pionex current-main validation PENDING"
    )
    dashboard["currentOperationsProjection"] = {
        "authority": False,
        "source": current_path,
        "reviewed_main_sha": str(current["reviewed_main_sha"]),
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "status": "PASS",
                "stage": "DASHBOARD_CURRENT_OPERATIONS_V0_2_OVERLAY_PASS",
                "schema": result["schema"],
                "reviewed_main_sha": current["reviewed_main_sha"],
