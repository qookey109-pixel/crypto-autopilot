...
class ToolkitHandler(BaseHTTPRequestHandler):
    server_version = "QookeyCryptoToolkitHTTP/0.1"
    ...
    def _write_json(self, status: int, payload: dict[str, Any]) -> None:
        ...
        cors_origin = os.getenv("QOOKEY_TOOLKIT_CORS_ORIGIN", "").strip()
        if cors_origin:
    ...
    def _check_auth(self) -> bool:
        ...
        self._write_json(
            401,
            {
                "schema": "qookey-crypto-toolkit-api-error-v0.1",
                "status": "ERROR",
    ...
    def do_OPTIONS(self) -> None:  # noqa: N802
        origin = os.getenv("QOOKEY_TOOLKIT_CORS_ORIGIN", "").strip()
        if not origin:
    ...
    def log_message(self, fmt: str, *args: object) -> None:
        ...
        print(f"toolkit-http {self.address_string()} {fmt % args}")
...
def main() -> int:
    ...
    print(f"Qookey Crypto Toolkit API listening on http://{host}:{port}")
