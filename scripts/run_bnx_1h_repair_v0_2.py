#!/usr/bin/env python3
"""Fresh-manual BNX repair entry point using the v0.2 repair bundle."""
...
from crypto_autopilot.history import bnx_repair_bundle_v0_2 as repair_bundle

ROOT = Path(__file__).resolve().parents[1]
RUNNER = ROOT / "scripts/run_binance_detailed_history.py"
spec = importlib.util.spec_from_file_location("bnx_repair_detailed_history_runner", RUNNER)
runner = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(runner)

# The existing writer remains the authority for serialization, R2 headroom,
...
def canonical_json_bytes(value):
    """Correct legacy repair receipt constants from pinned lineage before serialization."""
    ...
    for record in normalized.get("objects", []):
        ...
        if lineage.get("schema") not in {
            ...
            raise repair_bundle.RepairAuthorityError("BNX_REPAIR_BUNDLE_UNKNOWN_LINEAGE")
        ...
        if type(original_rows) is not int or original_rows <= 0 or original_audit is not False:
            raise repair_bundle.RepairAuthorityError("BNX_REPAIR_BUNDLE_LINEAGE_FIELDS_INVALID")
        record["source_archive_rows"] = original_rows
        record["source_archive_audit_ok"] = original_audit
        repaired += 1
    if repaired > 2:
        raise repair_bundle.RepairAuthorityError("BNX_REPAIR_BUNDLE_REPAIR_COUNT_EXCEEDED")
