...
AUTHORITY = ROOT / "research/receipts/2026-09-21-zec-v0-4-development-one-shot-authority.json"
CONTRACT = ROOT / "config/zec_strategy_v0_4_development_matrix_v0_1.json"
SELECTION_POLICY = ROOT / "config/zec_strategy_v0_3_selection_policy_v0_1.json"
WORKFLOW = "zec-v0-4-development-execution-v0-1.yml"
REPOSITORY = "qookey109-pixel/crypto-autopilot"
STEP_MS = 15 * 60 * 1000


class ZecV04ExecutionPreflightError(RuntimeError):
    pass
...
def _request_bytes(url: str, *, timeout_seconds: float = 45.0) -> bytes:
    """Perform exactly one bounded public-source request with no retry."""

    request = Request(
        url,
        headers={"User-Agent": "qookey-zec-v0-4-development/0.1"},
    )
    try:
...
def _github_json(url: str) -> dict[str, Any]:
    ...
    if not token:
        raise ZecV04ExecutionPreflightError("scoped GITHUB_TOKEN is required")
    request = Request(
        url,
        headers={
            ...
            "User-Agent": "qookey-zec-v0-4-authority-preflight/0.1",
        },
    )
    with urlopen(request, timeout=20.0) as response:  # noqa: S310 - fixed GitHub API host
        payload = json.load(response)
    if not isinstance(payload, dict):
        raise ZecV04ExecutionPreflightError("unexpected GitHub API response")
    return payload
...
def _verify_runtime_authority(
    ...
    *,
    ...
    if requested_authority_id != AUTHORITY_ID:
        raise ZecV04ExecutionPreflightError("workflow authority id mismatch")
    if os.environ.get("GITHUB_ACTIONS") != "true":
        raise ZecV04ExecutionPreflightError("GitHub-hosted execution is required")
    if os.environ.get("GITHUB_EVENT_NAME") != "workflow_dispatch":
        raise ZecV04ExecutionPreflightError("workflow_dispatch is required")
    if os.environ.get("GITHUB_REF") != "refs/heads/main":
        raise ZecV04ExecutionPreflightError("execution must run from protected main")
    if os.environ.get("GITHUB_REPOSITORY") != REPOSITORY:
        raise ZecV04ExecutionPreflightError("unexpected GitHub repository")
    if os.environ.get("GITHUB_RUN_ATTEMPT") != "1":
        raise ZecV04ExecutionPreflightError("one-shot authority forbids workflow reruns")

    run_id = os.environ.get("GITHUB_RUN_ID")
    head_sha = os.environ.get("GITHUB_SHA")
    if not run_id or not run_id.isdigit():
        raise ZecV04ExecutionPreflightError("valid GitHub run id is required")
    if (
        not isinstance(head_sha, str)
        or len(head_sha) != 40
        or any(ch not in "0123456789abcdef" for ch in head_sha)
    ):
        raise ZecV04ExecutionPreflightError("valid GitHub head SHA is required")

    pr_number = authority_payload.get("authority_pr_number")
    if isinstance(pr_number, bool) or not isinstance(pr_number, int) or pr_number < 1:
        raise ZecV04ExecutionPreflightError("authority PR number is not finalized")
    ...
    if (
        ...
        raise ZecV04ExecutionPreflightError("authority PR is not merged on main")
    ...
    if comparison.get("status") not in {"identical", "ahead"}:
        raise ZecV04ExecutionPreflightError(
            "current main does not descend from the authority merge"
        )

    title = f"ZEC V0.4 Development · {AUTHORITY_ID}"
    runs = _github_json(
    ...
    if prior:
        raise ZecV04ExecutionPreflightError(
            "one-shot ZEC V0.4 development authority is already consumed"
        )

    bindings = authority_payload.get("bound_git_blobs")
    if not isinstance(bindings, dict):
        raise ZecV04ExecutionPreflightError("authority Git blob bindings are missing")
    observed = {relative: _git_blob_sha(ROOT / relative) for relative in bindings}
    ...
    if not authority.effective:
        raise ZecV04ExecutionPreflightError("authority did not become effective")
    return authority, {
...
def _load_frozen_development_source() -> tuple[tuple[Candle, ...], list[dict[str, Any]]]:
    ...
    for period in _periods():
        ...
        if receipt.audit_ok is not True:
            raise RuntimeError("ZEC V0.4 requires strict full-month candle audit")
        if receipt.first_time_ms != start_ms:
            raise RuntimeError("ZEC V0.4 monthly archive does not start at month boundary")
        if receipt.last_time_ms != end_ms - STEP_MS:
            raise RuntimeError("ZEC V0.4 monthly archive does not end at month boundary")
        if receipt.row_count != expected_rows:
            raise RuntimeError("ZEC V0.4 monthly archive row count mismatch")
    ...
    if len(candles) != 140256:
        raise RuntimeError("ZEC V0.4 frozen development row count mismatch")
    if candles[0].time_ms != 1659312000000:
        raise RuntimeError("ZEC V0.4 frozen development start mismatch")
    if candles[-1].time_ms != 1785541500000:
        raise RuntimeError("ZEC V0.4 frozen development end mismatch")
    if any(
        right.time_ms - left.time_ms != STEP_MS
        for left, right in zip(candles, candles[1:])
    ):
        raise RuntimeError("ZEC V0.4 development source is not globally contiguous")
...
def execute(*, requested_authority_id: str) -> dict[str, Any]:
    ...
    results, ranking = run_zec_v0_4_development_matrix(
        candles_15m=candles,
        contract=contract,
        selection_policy_payload=selection_payload,
        execution_authority=execution_authority,
        funding_points=None,
    )
    candidates = build_zec_v0_4_candidate_grid(contract)
    fold_ids = tuple(str(row["fold_id"]) for row in contract["development_window"]["folds"])
    ...
    if len(results) != 24:
        raise RuntimeError("ZEC V0.4 development matrix is incomplete")
    ...
    return {
        "schema": "qookey-zec-v0-4-development-run-report-v0.1",
        ...
        json.dumps(
            {
                ...
                "provider_requests_performed": report["source"][
