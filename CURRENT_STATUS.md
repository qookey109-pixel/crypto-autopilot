...
| 3 | Core100 fingerprint V0.2 | PR #494 keeps the frozen V0.2 comparator PREPARED_NOT_ACTIVE. The non-authorizing cutover proposal merged in PR #501 at b150c965; it requires REVIEW_REQUIRED for a missing/legacy predecessor and separate authority before training or R2 access. A cloud-only collector prototype is being prepared in PR #502 from that main; active workflow and V0.1 remain unchanged. Next: validate the prototype and exact implementation blob inventory, then present the specific activation scope for user decision. |
