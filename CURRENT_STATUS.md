...
- Rollout acceptance remains **BLOCKED_PERMISSION / WAITING_TWO_NATURAL_RUNS (0/2 successful)**. Natural Health run `36099624753` triggered maintenance `36099661460`; inspect succeeded and propose failed with `BLOCKED_PERMISSION`. The next distinct natural Health run `36134440456` also succeeded and triggered maintenance `36134490577`; inspect succeeded and propose again failed with `BLOCKED_PERMISSION`. No successful maintenance round or `NO_CHANGE / commits_created=0` evidence exists. The GitHub Actions setting “Allow GitHub Actions to create and approve pull requests” remains a user decision; do not enable it or use manual dispatch/rerun to manufacture acceptance.
