...
Updated: 2026-09-25. This is the single human entrypoint for the current project checkpoint.
...
- Cloud Maintenance acceptance is **WAITING_TWO_POST_PERMISSION_NATURAL_RUNS (0/2)**. The pre-change natural rounds `36099624753` → `36099661460` and `36134440456` → `36134490577` remain historical `BLOCKED_PERMISSION` failures and do not count. On 2026-09-25, with user authorization, “Allow GitHub Actions to create and approve pull requests” was enabled; default `GITHUB_TOKEN` permissions remain read-only. No post-change natural Health source has run yet. Require two distinct future natural Health run IDs/attempts with complete inspect/propose outcomes, including at least one `NO_CHANGE / commits_created=0`; do not dispatch or rerun to manufacture evidence. Repository auto-merge and automatic head-branch deletion are enabled; auto-merge still requires an explicit per-PR opt-in after exact-head/base, required checks/reviews, and project authority gates pass.
