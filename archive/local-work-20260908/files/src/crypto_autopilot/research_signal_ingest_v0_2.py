...
from .online_r2_training import current_bucket_bytes
from .research_signal_layer import KOLForecast, deduplicate_kol_forecasts, validate_kol_forecast
...
def _validated_parse_mode(value: Any) -> str:
    if value not in ("structured_json_only", "metadata_only"):
        raise ResearchSignalIngestError("source parse_mode must be structured_json_only or metadata_only")
    return value
    ...
    *,
    ...
    for item in raw_forecasts:
        ...
        if type(published) is not int:
            raise ResearchSignalIngestError("forecast published_at_ms must be an explicit integer")
...
def collect_sources(
    ...
    *,
    ...
    # Validate the entire selected configuration before the first request.
