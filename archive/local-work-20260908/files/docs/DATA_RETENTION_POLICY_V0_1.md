...
Status: **BASE POLICY PREPARED_NOT_ACTIVE / FIXED DETAILED V0.1.1 AUTHORIZED
SEPARATELY AFTER V0.10 WINDOW**.
...
The rolling window described by `config/data_retention_policy_v0_1.json` would
be calculated at execution time as four years before the latest complete UTC
day, but that policy is not active. The current Detailed History V0.1.1
authority instead freezes `2022-08` through `2026-07`. In either design, a
market may start later than the requested window; the system must preserve its
actual listing/onset timestamp and must not pad history.
...
For the consolidated current inventory, including the distinction between
retained data, authorized-not-started data and frozen lanes, see
