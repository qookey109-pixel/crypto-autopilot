...
Local engineering note — 2026-09-07 (not a formal authority transition):
the [Daily Research Preview V0.1](docs/DAILY_RESEARCH_PREVIEW_V0_1.md) prepares
the Pionex-only shortlist view with synthetic inputs, existing candidate scores,
freshness/exclusion gates and no production integration. Broad historical data
remains strategy-research input, not the user-facing recommendation universe.
Pionex Paper resumption and eventual live trading still require separate authority.
...
### Operational alert — V0.10 scheduled execution absent

- Read-only GitHub Actions API observation at `2026-08-27T02:53:17Z` showed
  the V0.10 metadata workflow in `active` state and the frozen schedule present
  on `main`, but returned zero `schedule` runs. Six attempts were already due
  before the observation (`00:17`, `00:47`, `01:17`, `01:47`, `02:17`,
  `02:47` UTC).
- This is a missing-execution observation, not an R2 receipt evaluation. No
  provider, R2 or holdout data was read to produce it.
- The current operations authority requires missing slots to remain observed
  failures. Manual/retroactive backfill, a second execution path, silent source
  switching and holdout access remain unauthorized.
- The read-only health monitor's initial two-hour grace has expired; the local
  evaluator now classifies the exact no-run state as `STALE_NO_RUN`. Its next
  scheduled run can report the alert but cannot repair the incident.
- Consolidated data inventory and current issue list:
