...
Local product-direction follow-up (2026-09-07): broad historical research improves
strategies; the eventual daily shortlist and execution universe are Pionex-only.
An offline synthetic [daily research preview](docs/DAILY_RESEARCH_PREVIEW_V0_1.md)
now exercises ranking and rejection using existing Paper candidate semantics.
It is not a current investment recommendation, production integration or new authority.
...
history remains from `2020-01-01` to the latest complete UTC day, while USD-M
Detailed History V0.1.1 uses a fixed `2022-08` through `2026-07` window for
`15m`/`1h`/`4h` data. The separate rolling-retention policy remains prepared
and inactive. The current data inventory and the distinction between retained,
authorized-not-started and frozen lanes are indexed in
[`docs/DATA_OVERVIEW.md`](docs/DATA_OVERVIEW.md); the inactive proposal remains
documented in
...
4. [`docs/DATA_OVERVIEW.md`](docs/DATA_OVERVIEW.md) for the normalized current data inventory
5. [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)
6. [`docs/STRATEGY_V0_1.md`](docs/STRATEGY_V0_1.md) and [`config/strategy_v0_1.json`](config/strategy_v0_1.json)
...
GitHub Issues, specifications, tracer-bullet tickets and decision maps use the
planning-only bridge in
[`docs/ENGINEERING_WORKFLOW_V0_1.md`](docs/ENGINEERING_WORKFLOW_V0_1.md).
Their labels and status never grant provider, R2, holdout, model-promotion,
strategy-change or trading authority; the machine-readable planning contract is
[`config/engineering_workflow_v0_1.json`](config/engineering_workflow_v0_1.json).
