"""Bounded BNXUSDT 1h public-archive diagnosis; no R2 or repair path."""
...
EXPECTED_SHA = "b64fa8a52889378b7174cdf196029be4bb49b6f5255c3253f3645baaf25cb239"
CONFIG = "config/bnx_archive_diagnosis_v0_2.json"
CONFIG_SHA = "c4e3e788afb6be9c4e84de69b0fa645802595e2b18ae10be032ed52ea08b7a9e"
RECEIPT = "research/receipts/2026-09-12-bnx-archive-diagnosis-v0-2-authority.json"
KEY = BinanceVisionArchiveKey("klines", "monthly", "BNXUSDT", "1h", "2022-08")
STEP = 3_600_000
EXPECTED_ROWS = 744
OBSERVED_ROWS = 672
EXPECTED_MISSING = 72
...
def clock_gate(now=None):
    ...
    if not datetime(2026, 9, 12, tzinfo=timezone.utc) <= now < datetime(2026, 10, 1, tzinfo=timezone.utc):
...
def load_authority(root, env=None, now=None):
    ...
    if receipt != {
        "schema": "bnx-archive-diagnosis-authority-v0.2",
...
def validate_zip(key, data):
    with zipfile.ZipFile(io.BytesIO(data)) as archive:
        ...
        if (
            len(infos) != 1
            or infos[0].filename != key.csv_filename
            or infos[0].file_size > MAX_ARCHIVE_BYTES
        ):
    ...
    def __call__(self, url):
        ...
        try:
            with self.opener.open(
                Request(url, headers={"User-Agent": "crypto-autopilot-bnx-diagnosis-v0.2"}),
                timeout=30,
            ) as response:
...
def _utc_iso(time_ms):
    return datetime.fromtimestamp(time_ms / 1000, timezone.utc).isoformat().replace("+00:00", "Z")


def _raw_rows_by_time(key, data):
    rows = _csv_rows_from_archive(key, data)
    try:
        return {int(row[0]): tuple(row) for row in rows}
    except (IndexError, ValueError) as exc:
        raise DiagnosisError("RAW_ROW_TIMESTAMP_INVALID") from exc


def _verify_raw_overlap(monthly_rows, daily_key, daily_data):
    daily_rows = _raw_rows_by_time(daily_key, daily_data)
    overlaps = set(monthly_rows) & set(daily_rows)
    if not overlaps or any(monthly_rows[t] != daily_rows[t] for t in overlaps):
        raise DiagnosisError("RAW_OVERLAP_MISMATCH")
    return len(overlaps)
...
def diagnose(reader):
    ...
    for day in requested_days:
        key = BinanceVisionArchiveKey("klines", "daily", "BNXUSDT", "1h", day)
