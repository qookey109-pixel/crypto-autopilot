"""Bounded CTKUSDT 15m public-archive diagnosis; no R2 or repair path."""
...
EXPECTED_SHA = "bd77c44d0061511b2e8f5c140a06ad2dd30007c640586fad5b035d71b0c3c61c"
CONFIG = "config/ctk_archive_diagnosis_v0_1.json"
CONFIG_SHA = "110337d541627212a34dad28d691db887a63a4cb0d4ed4e6e27262fd7f42bb39"
RECEIPT = "research/receipts/2026-09-12-ctk-archive-diagnosis-v0-1-authority.json"
...
def clock_gate(now=None):
    ...
    if not datetime(2026, 9, 12, 7, 28, 29, tzinfo=timezone.utc) <= now < datetime(
...
def load_authority(root, env=None, now=None):
    ...
    if receipt != {
        "schema": "ctk-archive-diagnosis-authority-v0.1",
...
class PublicReader:
    ...
    def allow_day(self, key):
        if (
            ...
            or not key.period.startswith("2025-04-")
        ):
    ...
    def __call__(self, url):
        ...
        try:
            with self.opener.open(
                Request(url, headers={"User-Agent": "crypto-autopilot-ctk-diagnosis-v0.1"}),
                timeout=30,
            ) as response:
