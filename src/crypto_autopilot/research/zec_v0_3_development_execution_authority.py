...
SCHEMA = "qookey-zec-v0-3-development-one-shot-authority-v0.1"
STATUS = "PROPOSED_EFFECTIVE_ONLY_ON_EXPLICIT_PROTECTED_MAIN_PR_MERGE"
AUTHORITY_ID = "zec-v0-3-development-20260921-v0-1"
...
class ZecV03DevelopmentExecutionAuthority:
    authority_id: str
...
def validate_zec_v0_3_development_execution_authority(
    ...
    *,
    ...
    if payload.get("schema") != SCHEMA:
        raise ValueError("unexpected ZEC V0.3 development authority schema")
    if payload.get("status") != STATUS:
        raise ValueError("unexpected ZEC V0.3 development authority status")
    if payload.get("authority_id") != AUTHORITY_ID:
        raise ValueError("unexpected ZEC V0.3 development authority id")
    ...
    if payload.get("source_scope") != EXPECTED_SOURCE:
        raise ValueError("ZEC V0.3 one-shot source scope drifted")
    if payload.get("execution_scope") != EXPECTED_EXECUTION:
        raise ValueError("ZEC V0.3 one-shot execution scope drifted")
    if payload.get("safety_boundary") != EXPECTED_SAFETY_BOUNDARY:
        raise ValueError("ZEC V0.3 one-shot safety boundary drifted")
    ...
    if observed_blob_shas is not None:
        if dict(bindings) != dict(observed_blob_shas):
            raise ValueError("reviewed Git blob bindings do not match execution checkout")

    effective = bool(authority_pr_merged)
    return ZecV03DevelopmentExecutionAuthority(
