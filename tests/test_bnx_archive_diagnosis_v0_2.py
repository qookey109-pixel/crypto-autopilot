...
class BnxOneHourDiagnosisTests(unittest.TestCase):
    def setUp(self):
        self.start = int(datetime(2022, 8, 1, tzinfo=timezone.utc).timestamp() * 1000)
        self.times = list(range(self.start, self.start + 31 * 86_400_000, 3_600_000))
        self.missing = set(self.times[9 * 24:12 * 24])
        ...
        for day in (1, 10, 11, 12):
            key = BinanceVisionArchiveKey("klines", "daily", "BNXUSDT", "1h", f"2022-08-{day:02d}")
            payload, checksum = self.archive(
                key, self.times[(day - 1) * 24:day * 24]
            )
    ...
    def archive(key, timestamps, close="1.5"):
        csv_text = "\n".join(
            f"{time_ms},1,2,0.5,{close},1,{time_ms + 3_599_999},1,1,1,1,0"
    ...
    def test_candidate_proves_exact_three_day_gap_without_repair(self):
        expected_sha = hashlib.sha256(self.monthly).hexdigest()
        reader = self.reader()
        with patch.object(diagnosis, "EXPECTED_SHA", expected_sha):
            report = diagnosis.diagnose(reader)
        self.assertEqual(report["status"], "REPAIR_CANDIDATE_REQUIRES_PUBLICATION_AUTHORITY")
        self.assertEqual(report["observed_monthly_rows"], 672)
        self.assertEqual(report["candidate_row_count"], 744)
        self.assertEqual(report["inserted_rows"], 72)
        ...
        self.assertEqual(report["raw_overlap_rows_verified"], 24)
    ...
    def test_missing_daily_and_overlap_conflict_fail_closed(self):
        ...
        key = BinanceVisionArchiveKey("klines", "daily", "BNXUSDT", "1h", "2022-08-01")
        payload, checksum = self.archive(key, self.times[:24], close="1.6")
    ...
    def test_authority_is_exact_manual_main_and_time_bounded(self):
    ...
    def test_config_receipt_and_workflow_preserve_safety_boundary(self):
        ...
        self.assertEqual(config["provider"], "binance_usdm")
        self.assertEqual(config["symbol"], "BNXUSDT")
        self.assertEqual(config["interval"], "1h")
        self.assertEqual(config["period"], "2022-08")
        self.assertEqual(config["expected_missing_bars"], 72)
        for key in (
            "r2_access",
        ...
        workflow = (ROOT / ".github/workflows/bnx-archive-diagnosis-v0-2.yml").read_text()
        self.assertIn("workflow_dispatch:", workflow)
        self.assertIn("github.ref == 'refs/heads/main'", workflow)
        self.assertIn("python scripts/diagnose_bnx_archive_v0_2.py", workflow)
