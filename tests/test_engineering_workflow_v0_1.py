...
class EngineeringWorkflowV01Tests(unittest.TestCase):
    ...
    def test_current_main_and_versioned_authority_are_the_sources_of_truth(self) -> None:
        self.assertEqual(
            self.contract["source_of_truth"],
            ["live_repository_main", "current_versioned_protocol_config_and_receipt"],
        )
        self.assertEqual(
            self.contract["reading_order"][0:3],
            [
                "CURRENT_STATUS.md",
    ...
    def test_labels_are_optional_and_never_automated(self) -> None:
        policy = self.contract["label_policy"]
        self.assertTrue(policy["labels_are_optional_triage_metadata"])
        self.assertFalse(policy["automatically_applied"])
        self.assertFalse(policy["created_by_this_overlay"])
        self.assertFalse(policy["issue_or_pr_state_grants_authority"])

    def test_overlay_has_no_automatic_external_actions(self) -> None:
        self.assertTrue(self.contract["automatic_external_actions"])
        self.assertTrue(
            all(value is False for value in self.contract["automatic_external_actions"].values())
        )
    ...
    def test_agent_instructions_define_the_authority_bridge(self) -> None:
        ...
        self.assertIn(".github/ISSUE_TEMPLATE/work-item.yml", readme)
        self.assertIn("does not grant execution authority", readme)
