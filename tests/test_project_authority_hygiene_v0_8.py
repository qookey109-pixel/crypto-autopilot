...
V12_CONFIG = ROOT / "config/provider_equivalence_v0_12_successor_metadata_window_v0_1.json"
V12_WORKFLOW = (
...
def test_current_entry_docs_preserve_v08_v10_history_and_reflect_v12_successor() -> None:
    ...
    assert "V0.12 SUCCESSOR METADATA WINDOW" in status
...
def test_v12_is_unique_current_metadata_execution_owner_without_rewriting_history() -> None:
    ...
    assert not any(line == "  schedule:" for line in new_lines)
    assert any(line == "  schedule:" for line in v12_lines)
    assert v12["authorization_boundary"][
        "v0_10_scheduled_execution_authorized_after_main_merge"
    ] is False
    assert v12["authorization_boundary"][
        "v0_12_metadata_capture_execution_authorized_on_main_merge"
    ] is True
...
def test_dashboard_safe_fixture_reflects_v10_but_never_becomes_authority() -> None:
    ...
    assert project["currentMetadataCaptureExecutionPath"] == "github_hosted_ubuntu_v0_12"
