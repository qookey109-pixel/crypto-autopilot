...
class PionexHistoricalReachV03Tests(unittest.TestCase):
    ...
    def test_config_keeps_discovery_read_only_and_public(self) -> None:
        ...
        self.assertEqual(self.config["intervals"], ["15M", "60M", "4H", "1D", "1W"])
        ...
        self.assertEqual(self.config["maximum_requests"], 105)
        self.assertEqual(
            self.config["derived_timeframes"]["1Y"],
            {
    ...
    def test_short_page_plus_empty_probe_proves_provider_earliest(self) -> None:
        step = 4 * 60 * 60 * 1000
        rows = series(step, 1_200, _last_aligned_candle_before(self.cutoff, "4H"))
    ...
    def test_documented_cap_stops_at_exactly_ten_thousand(self) -> None:
        step = 4 * 60 * 60 * 1000
        rows = series(step, 11_000, _last_aligned_candle_before(self.cutoff, "4H"))
        client = FakeClient({"4H": rows})
        progress = {"requests": 0}
        result = discover_interval(self.config, client, "4H", progress)
    ...
    def test_gap_or_invalid_page_fails_closed(self) -> None:
        step = 4 * 60 * 60 * 1000
        rows = series(step, 600, _last_aligned_candle_before(self.cutoff, "4H"))
        del rows[-20]
        client = FakeClient({"4H": rows})
    ...
    def test_weekly_cursor_is_monday_utc_before_cutoff(self) -> None:
        ...
        self.assertEqual(result.classification, "PROVIDER_EARLIEST_REACHED")
        self.assertEqual(result.latest_time_ms, weekly)
    ...
    def test_all_interval_report_contains_metadata_not_candles(self) -> None:
        ...
        self.assertEqual(len(report["intervals"]), 5)
        self.assertEqual(
            [item["interval"] for item in report["intervals"]],
            ["15M", "60M", "4H", "1D", "1W"],
        )
        self.assertFalse(report["derived_timeframes"]["1Y"]["provider_native"])
        self.assertFalse(
            report["derived_timeframes"]["1Y"]["derivation_authorized_by_this_stage"]
        )
