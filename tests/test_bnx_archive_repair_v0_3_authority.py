"""Synthetic tests for BNX 4h repair authority and three-interval bundle; no provider or R2 access."""
...
from test_bnx_archive_diagnosis_v0_3 import BnxFourHourDiagnosisTests

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "config/bnx_archive_repair_v0_3.json"
RECEIPT = ROOT / "research/receipts/2026-09-12-bnx-archive-repair-v0-3-authority.json"
BUNDLE = ROOT / "config/bnx_archive_repair_bundle_v0_3.json"
BUNDLE_RECEIPT = ROOT / "research/receipts/2026-09-12-bnx-archive-repair-bundle-v0-3-authority.json"
BASE = ROOT / "config/binance_usdm_detailed_history_v0_1_2.json"
EVIDENCE = ROOT / repair_v0_3.EVIDENCE
NOW = datetime(2026, 9, 12, 4, 20, tzinfo=timezone.utc)
ENV = {
...
spec = importlib.util.spec_from_file_location(
    "bnx_repair_bundle_v0_3_wrapper", ROOT / "scripts/run_bnx_repair_bundle_v0_3.py"
)
...
class BnxFourHourRepairAuthorityTests(unittest.TestCase):
    def setUp(self):
        fixture = BnxFourHourDiagnosisTests()
        fixture.setUp()
        ...
        for day in (1, 10, 11, 12):
            key = diagnosis.BinanceVisionArchiveKey(
                "klines", "daily", "BNXUSDT", "4h", f"2022-08-{day:02d}"
            )
        ...
        self.partition = SimpleNamespace(
            ...
            interval="4h",
            period="2022-08",
            r2_key=repair_v0_3.DESTINATION,
            asset_class="crypto",
        )

    def test_authority_is_evidence_bound_and_manual_only(self):
        contract = repair_v0_3.load_contract(CONFIG, RECEIPT, BASE.read_bytes(), NOW, ENV)
        self.assertEqual(contract["candidate_rows"], 186)
        self.assertEqual(contract["inserted_rows"], 18)
        ...
        for key, value in (
            ...
            with self.subTest(key=key), self.assertRaises(repair_v0_3.RepairAuthorityError):
                repair_v0_3.load_contract(
                    CONFIG, RECEIPT, BASE.read_bytes(), NOW, dict(ENV, **{key: value})
                )
        with self.assertRaises(repair_v0_3.RepairAuthorityError):
            repair_v0_3.load_contract(CONFIG, RECEIPT, BASE.read_bytes() + b" ", NOW, ENV)
    ...
    def test_tampered_frozen_evidence_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "config/bnx_archive_repair_v0_3.json").write_bytes(CONFIG.read_bytes())
            evidence.write_bytes(EVIDENCE.read_bytes() + b" ")
            with self.assertRaisesRegex(repair_v0_3.RepairAuthorityError, "EVIDENCE_BINDING"):
                repair_v0_3.load_contract(
                    root / "config/bnx_archive_repair_v0_3.json",
                    RECEIPT,
                    BASE.read_bytes(),
                    NOW,
                    ENV,
    ...
    def test_reconstruction_matches_frozen_candidate_shape(self):
        ...
        with patch.object(repair_v0_3, "require_clock"):
            archive, lineage = repair_v0_3.reconstruct(
                self.partition, self.config, reader_factory=lambda: reader
            )
        self.assertEqual(len(archive.candles), 186)
        self.assertEqual(lineage["original_monthly_rows"], 168)
        self.assertEqual(lineage["inserted_rows"], 18)
        self.assertEqual(lineage["overlapping_rows_verified"], 6)
        self.assertFalse(lineage["original_monthly_audit_ok"])
        self.assertEqual(lineage["diagnosis_report_sha256"], repair_v0_3.EVIDENCE_SHA)

    def test_wrong_destination_blocks_before_provider(self):
        self.partition.r2_key = "not-authorized"
        factory = Mock()
        with patch.object(repair_v0_3, "require_clock"), self.assertRaises(
            repair_v0_3.RepairAuthorityError
        ):
            repair_v0_3.reconstruct(self.partition, self.config, factory)
        factory.assert_not_called()

    def test_bundle_binds_exactly_15m_1h_4h(self):
        ...
        self.assertEqual(loaded["v0_3"]["interval"], "4h")
        for interval in ("15m", "1h", "4h"):
            self.assertTrue(bundle.matches(SimpleNamespace(
                symbol="BNXUSDT", interval=interval, period="2022-08"
            )))
        self.assertFalse(bundle.matches(SimpleNamespace(
            symbol="BNXUSDT", interval="8h", period="2022-08"
        )))

    def test_wrapper_serializes_source_rows_for_v0_3_lineage(self):
        receipt = {
            "schema": "binance-usdm-detailed-history-shard-receipt-v0.1",
            "objects": [{
                "source_archive_rows": 186,
                "source_archive_audit_ok": True,
                "repair_lineage": {
        ...
        self.assertEqual(normalized["objects"][0]["source_archive_rows"], 168)
        self.assertFalse(normalized["objects"][0]["source_archive_audit_ok"])
        self.assertEqual(receipt["objects"][0]["source_archive_rows"], 186)
    ...
    def test_hashes_and_workflow_keep_new_authority_isolated(self):
        ...
        self.assertIn("--shard-index 3", new_repair)
        self.assertNotIn("schedule:", new_repair)
