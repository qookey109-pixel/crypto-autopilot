...
PRODUCER_AUTHORITY = ("external_source_fetch", "production_r2_write")
...
def _versioned_forbidden_authority_fields() -> set[str]:
...
def _authority_fixture(authority: object) -> FakeStore:
    """Keep lineage hashes valid so failures exercise the payload authority gate."""
    store = _fixture()
    latest_key = f"{NAMESPACE}/latest.json"
...
def _legacy_authority() -> dict[str, object]:
    return {
        **dict.fromkeys(REQUIRED_FALSE_AUTHORITY, False),
        **dict.fromkeys(PRODUCER_AUTHORITY, True),
    }


class ResearchSignalQualityTests(unittest.TestCase):
    def test_versioned_forbidden_fields_reject_non_false_values(self) -> None:
        for field in sorted(_versioned_forbidden_authority_fields()):
            for value in (True, None, 0, 1, "false", [], {}):
                with self.subTest(field=field, value=value):
                    store = _authority_fixture({**_legacy_authority(), field: value})
                    with self.assertRaisesRegex(
                        ResearchSignalQualityError, "unexpectedly authorizes"
                    ):
                        evaluate_research_signal_quality(
                            store, namespace=NAMESPACE, now=NOW
                        )
                    self.assertEqual(len(store.reads), 3)

    def test_optional_explicit_denials_are_compatible(self) -> None:
        explicit_denials = {
            **_legacy_authority(),
            **dict.fromkeys(_versioned_forbidden_authority_fields(), False),
        }
        for authority in (_legacy_authority(), explicit_denials):
    ...
    def test_required_denials_cannot_be_omitted(self) -> None:
    ...
    def test_producer_flags_are_boolean_and_do_not_grant_evaluator_access(self) -> None:
        for field in PRODUCER_AUTHORITY:
            for value in (True, False, None, 0, 1, "false", [], {}):
                with self.subTest(field=field, value=value):
                    store = _authority_fixture({**_legacy_authority(), field: value})
                    if type(value) is not bool:
                        with self.assertRaisesRegex(
                            ResearchSignalQualityError, "must be boolean"
                        ):
                            evaluate_research_signal_quality(
                                store, namespace=NAMESPACE, now=NOW
                            )
    ...
    def test_unknown_authority_fields_fail_closed_without_echoing_input(self) -> None:
        for value in (True, False, None, 0, 1, "false", [], {}):
            with self.subTest(value=value):
                store = _authority_fixture(
                    {
                        **_legacy_authority(),
                        "synthetic_unrecognized_permission": value,
                    }
                )
                ...
                self.assertEqual(
                    ...
                    "signal payload contains unknown authority fields",
                )

    def test_malformed_payload_authority_fails_closed(self) -> None:
