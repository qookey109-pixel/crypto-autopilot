...
### V0.10 Pionex PERP query emergency — PROPOSED IN PR #210 / NOT EFFECTIVE

- Three consecutive scheduled captures failed closed because the existing
  Pionex symbols request omitted the endpoint's market `type`; the documented
  default is Spot, while the frozen scope contains 15 perpetual symbols.
- PR #210 adds exactly `type=PERP` to the existing public Pionex request and
  rejects a conflicting pre-existing type. Binance, Render, the schedule,
  frozen symbols/pairs, freshness and the 8 GB R2 hard stop are unchanged.
- The change applies only to future scheduled attempts after protected-main
  merge and required CI. It does not validate, replay or backfill any prior
  failed or missing slot, and it makes no provider request during review.
- Authority/config:
  `config/v0_10_mid_window_emergency_pionex_perp_query_v0_1.json`; receipt:
  `research/receipts/2026-08-30-v0-10-mid-window-emergency-pionex-perp-query-authority.json`.
- No holdout access, source switch, provider splicing, strategy/model change,
  manual capture, trade plan, order or live trading is authorized.
